...
import cityAllList from '../../files/json/city.list.tw.json';
...
class SearchBar extends Component {
	...
	componentWillMount() {
		document
		.querySelector('body')
		.addEventListener('click', () => this.setState({ autocompleteShow: false }))
	}

	showAutocomplete(e) {
		const searchText = e.target.value;

		this.setState({ town: searchText, fireRedirect: false, autocompleteShow: true });

		// input 沒有文字不用顯示 auto
		if (searchText.length === 0) {
			this.setState({ cityAutoData: [], autocompleteShow: false });
		} else {
			...
			if (cityAutoData.length === 0) {
				...
				this.setState({ cityAutoData, autocompleteShow: true }, () => {
	...
	onKeyDown(e) {
		...
		(e.key === 'Escape') && this.setState({ autocompleteShow: false });
	...
	selectText(e) {
		...
		this.setState({ town, city, fireRedirect: false, autocompleteShow: false });
	}
		...
		return (
			<div className="public-container-search">
				<form onSubmit={e => this.formSubmit(e)} autoComplete="off">
					...
					<input 
						type="text" name="town" 
						onChange={e => this.showAutocomplete(e)} 
				...
				}
				{
					this.state.autocompleteShow
						? <Autocomplete 
							cityAutoData={this.state.cityAutoData} 
							selectTextCB={e => this.selectText(e)} 
							onMouseOverCB={e => this.onMouseOver(e)} />
						: null
				}			
			</div>
		)
