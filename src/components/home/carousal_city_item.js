...
const CarousalCityItem = ({carousal}) => {
	...
	return (
		<Link className="index-container-cityInfo-detail" {...weatherDetailLink}>
			<h2>{(dataEmpty)? 'loading...': `${carousal.town.cate.name} ${carousal.town.name}`}</h2>
			<div className="index-container-cityInfo-detail-each">
				<span className="index-container-cityInfo-detail-each-title">天氣狀況</span>
				<span className="index-container-cityInfo-detail-each-value">{(dataEmpty)? 'loading...': carousal.weather.desc}</span>
			</div>
			<div className="index-container-cityInfo-detail-each">
				<span className="index-container-cityInfo-detail-each-title">溫度</span>
				<span className="index-container-cityInfo-detail-each-value">{(dataEmpty)? 'loading...': `${carousal.weather.temperature} °C`}</span>
			</div>
			<div className="index-container-cityInfo-detail-each">
				<span className="index-container-cityInfo-detail-each-title">濕度</span>
				<span className="index-container-cityInfo-detail-each-value">{(dataEmpty)? 'loading...': `${carousal.weather.humidity} %`}</span>
			</div>
			<div className="index-container-cityInfo-detail-each">
				<span className="index-container-cityInfo-detail-each-title">雨量</span>
				<span className="index-container-cityInfo-detail-each-value">{(dataEmpty)? 'loading...': `${carousal.weather.rainfall} mm`}</span>
			</div>
